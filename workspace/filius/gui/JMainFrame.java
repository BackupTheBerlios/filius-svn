...
//							  + "\tsourceType: '"+e.getSource().getClass().getSimpleName()+"'\n");
            	 ...
            	 if( (e.getKeyChar() == KeyEvent.VK_SPACE) && 
            		 ...
            		 return true;   // no further action by 'clicking' involuntarily via the space bar; 
            		 				// other shortcuts will be given for this 
            	 }
            	 /* delete item on deletion key press */
            	 if( (e.getKeyChar() == KeyEvent.VK_DELETE) &&
            		 !(e.getSource().getClass().getSimpleName().equals("JTextField")) &&
            		 (frame.isFocused())) {
            		 if(GUIContainer.getGUIContainer().getMarkierung().isVisible()) {   // several items are selected
    					...
    					while (it.hasNext()) {
    						...
    						if (   tx 
    								> auswahl.getX() - GUIContainer.getGUIContainer().getAbstandLinks()
    							...
    							&& tx + twidth 
    							    < auswahl.getX()
    										...
    										+ scrollPane.getHorizontalScrollBar()
    												.getValue()
    							&& ty 
    								> auswahl.getY() - GUIContainer.getGUIContainer().getAbstandOben()
    										+ scrollPane.getVerticalScrollBar().getValue()
    							&& ty + theight 
    								< auswahl.getY() + auswahl.getHeight()
...
//    						Main.debug.println("\t"+((GUIKnotenItem) markedlist.get(i)).getKnoten().getName());
            		 ...
            		 else if(GUIEvents.getGUIEvents().getActiveItem() != null) {  // single item active
//            			 Main.debug.println("KeyDispatcher:  delete item '"+(GUIEvents.getGUIEvents().getActiveItem()!=null ? 
//            						 GUIEvents.getGUIEvents().getActiveItem().getKnoten().getName() : "<null>"));
            		 ...
            		 //else Main.debug.println("DEL pressed, but nothing selected");
            	 }  // del key
...
//                	 		Main.debug.println("KeyDispatcher:    CTRL+H recognised");
  ...
  //              	 		Main.debug.println("KeyDispatcher:    CTRL+A recognised");
                 ...
                 if(e.getModifiers() == 8) {   // ALT key pressed; only makes sense for cables!
                	 if (e.getKeyCode() == 49) { // key '1'  (cable)
//                	 		Main.debug.println("KeyDispatcher:    ALT+1 recognised");
                	 		GUIContainer.getGUIContainer().getKabelvorschau().setBounds(
        							...
        							- (GUIContainer.getGUIContainer().getKabelvorschau().getHeight() / 2) -32,   // subtract titlebar (approx. 32px)
