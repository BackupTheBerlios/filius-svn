...
import filius.rahmenprogramm.WinFolders;
...
public class Information implements Serializable {
	...
	/** Pfad zum Verzeichnis, in dem das Programm ausgefuehrt wird
	 * (in dem sich die ausfuehrbare Jar-Datei befindet); <br />
	 * der Pfad schliesst mit dem Pfad-Trennzeichen (unter UNIX "/")
	 */
	private String programmPfad = null;
	
	/** Pfad zum Verzeichnis, in dem die benutzerspezifischen Daten
	 * abgelegt werden; <br />
	 * der Pfad schliesst mit dem Pfad-Trennzeichen (unter UNIX "/")
	 */
	...
	// actually used working directory, i.e., path to be used after initial tests
	private String arbeitsbereichPfad = getHomeDir() //System.getProperty("user.home")
	+ System.getProperty("file.separator") + ".filius"
	+ System.getProperty("file.separator");
...
//////////////////////////////////////
       ...
       /**
        * ensure a correct and well functioning path to write all
        * necessary data
        */
        private boolean checkWD(String currPath) {
          ...
          do {
            try {
              testFile = new java.io.File(currPath+".filius"+System.getProperty("file.separator")+randomString);
	      //
	      // write check, i.e., create random directory and delete it again
              if(!testFile.mkdirs()) { // test, whether dir is writeable
	        throw new Exception("EXCEPTION: Error on making directories in write-check");
	      }
	      ...
	      javax.swing.JOptionPane.showMessageDialog(null, "Fehler: Verzeichnis ist nicht schreibbar. Filius benötigt aber Schreibrechte.\n"+
          ...
          arbeitsbereichPfad=currPath + ".filius" + System.getProperty("file.separator");  // set correct path
          return true;
        }
	...
	public static Information getInformation(String path) {
		if (information == null) {
			if(path != null) information = new Information(path);
			else information = new Information();
		}
                if(information.initOk) return information;
                else return null;
	}
	...
	/**
	 * Hier werden die Verzeichnisse und die Datei mit den Informationen
	 * zu den eigenen Anwendungen erstellt, wenn sie noch nicht existieren:
	 * <ul>
	 *   <li> Arbeitsbereich: Verzeichnis, in dem alle benutzerspezifischen
	 *   Daten gespeichert werden </li>
	 ...
	 *   werden mit den Unterordnern software/clientserver/ und gui/anwendungssicht/
	 *   </li>
	 *   <li> Datei, in der die Einstellungen zu eigenen Anwendungen gespeichert werden
	 *   </li>
	 * </ul>
	 */
	private void initialisiereVerzeichnisse() throws FileNotFoundException, IOException {
		...
		if (!(new java.io.File(pfad)).exists())
			(new java.io.File(getAnwendungenPfad()
						+ "EigeneAnwendungen.txt")).createNewFile();
	}
	...
	public LinkedList<HashMap<String, String>> ladePersoenlicheProgrammListe() throws IOException {
		...
		for (String line; (line = desktopFile.readLine()) != null;) {
			...
			if (desktopFile != null) desktopFile.close();
		}
	...
	public String getProgrammPfad() {
		if (programmPfad != null) {
			...
			if(str.indexOf("filius.jar")>=0) {   // run from jar file
				if((new File(str)).isAbsolute()) programmPfad="";   // in case of absolute path, delete "user.dir" entry
				// da Java beim Aufruf verschiedene Separatoren unterstützt, wird hier getrennt abgefragt...
				if(str.indexOf("/")>=0) {
	...
	public String getRelativePathToProgramDir() {
		...
		if(File.separator.equals("\\")) {
			if(progPath.substring(1,3).equals(workPath.substring(1,3))) {   // directories on same drive --> remove drive letter
		...
		if (progPath.indexOf(workPath)==0) {
			if (progPath.length()<=workPath.length()) progPath="";
			else progPath = progPath.substring(workPath.length());
			workPath = "";
		}
		else if (workPath.indexOf(progPath)==0) {  // otherway round
			if (workPath.length()<=progPath.length()) workPath="";
			else workPath = workPath.substring(progPath.length());
			progPath = "";
		}
		...
		while (!finished) {
			...
			if(slashIdx>=0) {   // subdirectories left to be stepped up via "../" strings
				relativePath+=".."+File.separator;
				if(workPath.length()<=slashIdx+2) workPath = "";
				else workPath = workPath.substring(slashIdx+1);
			}
	...
	public void setArbeitsbereichPfad(String otherWD) {
		...
		else arbeitsbereichPfad = otherWD + System.getProperty("file.separator");   // no separators, but possibly still legitimate String
