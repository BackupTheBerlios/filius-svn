 ...
 * 
 *         Der emailServer muss wohl neu aufgerollt werden, da ich hier
 *         überhaupt kein Thread brauche. Dies wird durch POP3- und SMTPServer
 *         realisiert. Daher wird der nun überarbeitet. 13.12.2006
 */
public class EmailServer extends Anwendung implements I18n {
	...
	/**
	 * Ob der Server eingehende Verbindungsanfragen annmimmt. Das gilt sowohl
	 * fuer SMTP wie auch fuer POP3
	 */
	...
	/**
	 * Ob der Server eingehende Verbindungsanfragen annmimmt. Das gilt sowohl
	 * fuer SMTP wie auch fuer POP3. <br />
	 * In dieser Methode wird also sowohl das Attribut dieser Klasse gesetzt,
	 * das fuer SMTP und POP3 gilt, wie auch die jeweiligen Attribute des
	 * POP3Server und des SMTPServer.
	 */
	...
	/**
	 * Hier wird die ueberladene Methode der Oberklasse aufgerufen, die
	 * Benutzerkonten gespeichert und POP3Server und SMTPServer beendet.
	 */
	public void beenden() {
	...
	/**
	 * Diese Methode wird beim Wechsel vom Entwurfs- in den Aktionsmodus
	 * aufgerufen. Hier wird zuerst die ueberladene Methode der Oberklasse
	 * aufgerufen. Dann werden die Benutzerkonten aus der Datei des Servers
	 * geladen. Danach wird ein neuer POP3Server und ein neuer SMTPServer
	 * erzeugt und initialisiert. D. h., dass sie auch deren starten()-Methode
	 * aufgerufen wird und das Attribut 'aktiv' gesetzt wird, das angibt, ob ein
	...
	/**
	 * FUNKTIONIERT In dieser Methode wird ein neuer Benutzer hinzugefuegt, d.h.
	 * ein neues Konto in der Liste "listeBenutzerkonten" aller Nutzer erstellt.
	 * Dazu muessen der Methode alle für einen Account relevanten Daten
	 * uebergeben werden. Zunaechst wird dann abgefragt, ob es in dieser Liste
	 * bereits den Benutzernamen gibt, den der Client für sein Konto moechte,
	 * wenn ja, gibts einen Fehler, wenn nein, ist er akzeptiert. Hier ist
	 * darauf zu achten, dass sich der Benutzername aus dem benutzernamen + "@"
	 * + Name des Email- Servers zusammensetzt. Dies muss explizizt abgefragt
	 * werden (siehe letzter Satz) Danach muss abgefragt werden, ob nicht alle
	 * anderen Felder wie Vorname, Strasse, usw. auch bereits einem
	 * existierenden Kontakt in der Liste entsprechen. Unterscheiden sich diese
	 * ganzen sekundaeren Angaben in mindestens einem Punkt, kann der Kontakt
	 * schliesslich vervollstaendigt und der "listeBenutzerkonten" hinzugefuegt
	 * werden. Der Benutzername ent- spricht dann dem gewuenschten Begriff oder
	...
	/**
	 * FUNKIONIERT Diese Methode loescht einen Benutzeraccount aus der Liste
	 * "listeBenutzerkonten" des EmailServers. Dazu wird die gesamte Liste
	 * durchlaufen und nach dem entsprechenden Benutzernamen gesucht, der
	 * geloescht werden soll. Danach wird das Passwort abgefragt, die auch als
	 * Parameter der Methode uebergeben werden. Sind beide richtig, wird das
	 * Konto aus der Liste entfernt. Klappt das wird true zurückgegeben,
	 * andernfalls eine Exception geworfen. Hier muss auch wieder beachtet
	 * werden dass sich der vollständige Benutzername aus dem eigentlichen
	 * benutzernamen, den der User angibt, + "@" + Name des EmailServers ergibt.
	 * Der User muss aber nur seinen Benutzernamen eingeben, der wird dann
	 * vervollständigt.
	 * 
	 * @param benutzername
	 * @param passwort
	...
	/**
	 * FUNKTIONIERT Hier wird ein EmailKonto zu einer bestehenden Verbindung
	 * gesucht. Es wird eine VerbindungsId, die beim Aufbau einer neuen
	 * Verbindung gespeichert wird, uebergeben. Mit ihr wird zunaechst die Liste
	 * aller Ver- bindungen durchlaufen, und abgefragt, ob es eine Verbindung
	 * mit dieser uebergebenen Id gibt. Trifft das zu, wird aus ihr der
	 * Benuztername(bname) und das Passwort herausgelesen und temporaer in der
	 * Methode ge- speichert. Mit diesen beiden nun wird die
	 * "listeBenutzerkonten", die Liste aller EmailPostfächer, bzw.
	 * registrierter Benutzer dieses EmailServers durchlaufen, um zu suchen, ob
	 * es einen Nutzer gibt, der dieses Konto mit dem bname und Passwort hat.
	 * Gibt es das, wird dieses Konto zurueckgegeben.
	 * 
	 * @param verbindungsId
	 * @return EmailKonto
	 */
	...
	/**
	 * NACHTRAG 2008: Methode ist void, gibt aber boolean erfolg aus, außerdem
	 * wird sie in "nachrichtenKommunikation" aufgerufen, die selbst keinerlei
	 * Aufgabe mehr hat! Hier werden die Benutzerkonten zunaechst in einer Datei
	...
	/**
	 * NACHTRAG 2008: 2te "while-Schleife" scheint ueberfluessig, scheint
	 * dennoch zu gehen. Hier wird ein Benutzerkonto aus der Liste der
	 * Benutzerkonten in einen String umgewandelt. Der String hat die Form
	 * benutzername+kontoDomain+passwort+nachname+vorname+nachrichten. Dies
	 * brauche ich fuer SPEICHERUNG
	 * 
	...
	/**
	 * Das brauche ich fuers LADEN, ist der Gegensatz zu
	 * listeBenutzerkontenZuString.
	 * 
	 * @param speicherung
	 * @return
	 */
	private LinkedList stringZuListeBenutzerkonten(String speicherung) {
		...
		while (speicherungTokenizer.hasMoreTokens()) {
			...
			if (emailKontoInStringTokenizer.hasMoreTokens()) {
				...
				// emailStr = emailStr+"$"+
				// email.getAbsender()+"$"+email.getEmpfaenger()+"$"+email.getDateReceived()+"$"+
				...
				try {
					...
					for (int i = 0; i < nachrichtenArray.length; i++) {
						...
						// else throw new
						// Exception("mail string has more/less than 7 tokens (="+strArray.length+")!");
					}
				} catch (Exception e) {
					Main.debug
					        .println("EXCEPTION ("
					                ...
					                + ") "
					                + getClass()
					                + " (EmailServer), stringZuListeBenutzerkonten:  mail does not have enough or too many entries");
	...
	public void setSystemSoftware(InternetKnotenBetriebssystem bs) {
		 ...
		 */
		boolean check/* this.verzeichnis */= getSystemSoftware().getDateisystem().erstelleVerzeichnis(
		        getSystemSoftware().getDateisystem().getRoot(), "mailserver");
		this.verzeichnis = getSystemSoftware().getDateisystem().verzeichnisKnoten(
		        getSystemSoftware().getDateisystem().getRoot(), "mailserver");
	}
	 ...
	 * verwende Methode aufgerufen wird. 1. erstellen eines neuen SMTPClienten
	 * 2. starten des Clienten 3. versenden Es wird die Email übergeben und
	 * genau ein Empfaenger, an den die Email weiter- geleitet werden soll. Dies
	 * wird so oft wiederholt in "verarbeiteEmail", bis alle Empfaenger, an die
	 * weitergeleitet werden soll, durchgelaufen sind. <br />
