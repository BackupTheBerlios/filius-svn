...
/**
 * Der Mitarbeiter dient dazu, den Datenaustausch mit einem Client
 * durchzufuehren. <br />
 * Dazu durchlaeuft er folgende Zustaende:
 * <ol>
 * <li>START: Empfang des Begruessungskommandos (HELO oder EHLO)</li>
 * <li>ENVELOPE: Empfang der 'Briefumschlags'-Informationen (Sender, Empfaenger
 * etc)</li>
 ...
 * Mitarbeiter im END-Zustand. Das heisst, dass dieser Mail-Server es nicht
 * ermoeglicht mit einer Verbindung mehrere E-Mails zu uebertragen.
 * 
 */
public class SMTPMitarbeiter extends ServerMitarbeiter {
	...
	/**
	 * In dieser Methode werden die ankommenden Befehle und Daten zur
	 * Uebertragung einer E-Mail verarbeitet.
	 */
	 ...
	 * Methode in Abhaengigkeit der angegebenen Empfaenger in einem lokalen
	 * Postfach abgelegt bzw. an einen neuen Empfaenger weiter geleitet.
	 */
	...
	/**
	 * Methode prüft, ob die Domain, an die die Email geschickt werden soll, mit
	 * der des eigenen MailServers übereinstimmt.
	 * 
	 * @param str
	 * @return boolean
	 */
	private boolean pruefeAufSelbeDomain(String str) {
		Main.debug.println("INVOKED (" + this.hashCode() + ", T" + this.getId() + ") " + getClass()
		        + " (SMTPMitarbeiter), pruefeAufSelbeDomain(" + str + ")");
		boolean check = false;
	...
	private void zeigeEmail(Email email) {
		Main.debug.println("INVOKED (" + this.hashCode() + ", T" + this.getId() + ") " + getClass()
		        + " (SMTPMitarbeiter), zeigeEmail(" + email + ")");
